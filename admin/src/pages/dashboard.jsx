...
const Dashboard = ({ token }) => {
  ...
  const totalSales = orders.reduce(
    (acc, order) => acc + (order.payment ? order.amount : 0),
    0
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {orders.slice(0, 5).map((order, index) => (
            <div key={index} className="border-b py-2 text-sm text-gray-700">
              <div className="flex justify-between">
                <span>
                  {order.address.firstName} ({order.paymentMethod})
                </span>
        ...
        <div className="bg-white rounded-xl shadow p-4">
          ...
          {products.slice(0, 5).map((product, index) => (
            <div
              key={index}
              className="flex justify-between items-center py-2 border-b text-sm text-gray-700"
            >
